...
function Row({ label, value, bold, accent }: RowProps) {
  ...
  return (
    <div className={`flex justify-between py-1.5 border-b border-gray-100 ${bold ? 'font-bold' : ''}`}>
...
export default function PayslipDetailView({ payslip }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div className="bg-brand-700 text-white rounded-xl px-5 py-4">
        ...
        <p className="text-2xl font-bold mt-2 tabular-nums">{formatYen(summary.netPay)}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-brand-600 mb-3">支給</p>
        ...
        {/* Deductions */}
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-red-500 mb-3">控除</p>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: '遅早時間', value: attendance.lateEarlyHours, unit: 'h' },
            { label: '休日出勤', value: attendance.holidayWorkDays, unit: '日' },
          ].map((item) => (
            item.value > 0 ? (
