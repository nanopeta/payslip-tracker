...
const ATTENDANCE_MAP: Array<[string, AttendanceKey]> = [
  ['出勤日数', 'workDays'],
...
function parsePayslipTokens(tokens: string[]): Partial<Payslip> {
  ...
  for (const [label, key] of ATTENDANCE_MAP) {
    const v = findValueAfter(tokens, label)
...
export async function parsePDF(file: File): Promise<ParseResult> {
  ...
  if (type === 'payslip') {
    ...
    const totalFields = INCOME_MAP.length + DEDUCTION_MAP.length + ATTENDANCE_MAP.length + 2
    const successFields =
