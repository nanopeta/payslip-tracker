...
## 利用可能なツール（Claude Code 組み込み）
- Read: ファイルを読む
- Edit / Write: ファイルを編集・作成する
- Bash: コマンドを実行する（npm run build, git diff, git log など）
- git commit/push/add は Bash から実行禁止（スクリプト側が管理）
`
...
## スプリント計画の手順
1. Bash ツールで \`git log --oneline -30\` を実行して実装済みの改善を確認
2. Read ツールで CLAUDE.md を読んで最新の制約・設計方針・コンポーネント一覧を把握
3. Bash ツールで \`ls src/pages src/components/charts\` を確認
...
## 実装手順
1. Read ツールで CLAUDE.md を読んで最新の制約を確認
2. Read ツールで変更予定のファイルを読む
3. 類似する既存コンポーネント（TrendSummaryChart.tsx, StatCard.tsx 等）も Read ツールで読んで一貫性を保つ
4. Edit または Write ツールで変更を適用
5. Bash ツールで \`npm run build\` を実行してエラーがないか確認
...
- Bash ツールで git commit/push/add を実行しない`

const REVIEWER_SYSTEM = `あなたは payslip-tracker のコードレビュアーです。
${BASE_CONSTRAINTS}

## レビュー手順
1. Bash ツールで \`git diff HEAD\` を実行して変更内容を確認
2. 変更されたファイルを Read ツールで詳しく確認
3. チェック項目:
...
## 手順
1. Bash ツールで \`git diff HEAD\` を実行してスプリントの変更内容を確認
2. Read ツールで CLAUDE.md を読む
3. 必要なら Edit ツールで CLAUDE.md を更新:
